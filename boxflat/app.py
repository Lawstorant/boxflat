...
class MyApp(Adw.Application):
    def __init__(self, data_path: str, config_path: str, dry_run: bool, custom: bool, autostart: bool,**kwargs):
        ...
        self._description_handler = DescriptionHandler(data_path)
    ...
    def _prepare_settings(self):
        self._panels["Home"] = HomeSettings(self.switch_panel, self._dry_run, self._cm, self._hid_handler, self._version, self._description_handler)
        self._panels["Base"] = BaseSettings(self.switch_panel, self._cm, self._hid_handler, self._description_handler)
        self._panels["Dash"] = DashSettings(self.switch_panel, self._cm, self._hid_handler, self._settings, self._description_handler)
        self._panels["Wheel"] = WheelSettings(self.switch_panel, self._cm, self._hid_handler, self._settings, self._description_handler)
        self._panels["Wheel Old"] = OldWheelSettings(self.switch_panel, self._cm, self._hid_handler, self._settings, self._description_handler)
        self._panels["Pedals"] = PedalsSettings(self.switch_panel, self._cm, self._hid_handler, self._description_handler)
        self._panels["H-Pattern Shifter"] = HPatternSettings(self.switch_panel, self._cm, self._settings, self._hid_handler, self._description_handler)
        self._panels["Sequential Shifter"] = SequentialSettings(self.switch_panel, self._cm, self._hid_handler, self._description_handler)
        self._panels["Handbrake"] = HandbrakeSettings(self.switch_panel, self._cm, self._hid_handler, self._description_handler)
        self._panels["Multifunction Stalks"] = StalksSettings(self.switch_panel, self._cm, self._hid_handler, self._settings, self._description_handler)
        self._panels["Universal Hub"] = HubSettings(self.switch_panel, self._cm, self._description_handler)

        self._panels["Presets"] = PresetSettings(self.switch_panel, self._cm, self._settings, self._panels["H-Pattern Shifter"], self._panels["Multifunction Stalks"], self._description_handler)
        self._panels["Presets"].set_application(self)
        self._panels["Generic Devices"] = GenericSettings(self.switch_panel, self._settings, self._description_handler)

        self._panels["Other"] = OtherSettings(
            self.switch_panel, self._cm, self._hid_handler, self._settings, self._version, self, self._data_path, self._description_handler)
